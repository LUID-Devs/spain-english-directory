...
<html lang="en">
  ...
  <body>
    <div id="root">${data.content}</div>
      ...
      // IMPORTANT: Do NOT overwrite the root index.html
      // The root index.html is the SPA shell generated by Vite, used for:
      // - Client-side routes like /dashboard (authenticated pages)
      // - Fallback when no prerendered content exists
      // 
      // The landing page is available at /landing (prerendered above)
      // and the root / route renders LandingPage via client-side routing
      console.log(`[static-prerender] Preserving SPA shell at /index.html for client-side routes`);
    }
  };
}
