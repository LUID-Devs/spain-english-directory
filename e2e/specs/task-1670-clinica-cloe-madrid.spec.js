...
test.describe('TASK-1670: Clinica Cloe Dental Madrid', () => {
  ...
  test('should verify Clinica Cloe has English-speaking indication', async ({ page }) => {
    ...
    // Verify Clinica Cloe is listed
    await expect(page.locator('text=Clinica Cloe')).toBeVisible();
  });

  test('should find Clinica Cloe with dental specialty filter', async ({ page }) => {
    await page.goto('/madrid/dentists');
    await page.waitForLoadState('networkidle');
    
    // Verify the clinic is visible on the dentists page
    await expect(page.locator('text=Clinica Cloe')).toBeVisible();
    
    // Verify it has dental-related content
    await expect(page.locator('text=Dental')).toBeVisible();
  });
