...
export function useHybridSearch(options: UseHybridSearchOptions = {}): UseHybridSearchReturn {
  ...
  const performSearchRef = useRef<(searchQuery: string, searchLimit?: number) => Promise<void>>(async () => {});
  ...
  // Keep performSearchRef current to avoid stale closure
  useEffect(() => {
  ...
  const setQuery = useCallback((newQuery: string) => {
    ...
    if (newQuery.trim().length >= minQueryLength) {
      debouncedSearch(newQuery, defaultLimit);
    } else {
      setResults([]);
      setTotalCount(0);
      setError(null);
    }
  }, [debouncedSearch, minQueryLength, defaultLimit]);
